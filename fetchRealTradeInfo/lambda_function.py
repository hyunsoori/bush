...
engine = create_engine('mysql://bush_app_user:a1234567@bushdbinstance.csaae2uji9ic.ap-northeast-2.rds.amazonaws.com/bush?charset=utf8mb4')
	...
	for f in asyncio.as_completed(fts):
		...
		for idx, item in enumerate(r['response']['body']['items']['item']):
			try:
				...
				apt_trade_info = AptTradeInfo(key=key, yyyymm=yyyymm, days=item['일'], city_code=city_code, city_name=seoul_area_code[int(city_code)], dong_name=item['법정동'], apt_name=item['아파트'], floor=int(item['층']), space=item['전용면적'], price=price, start_year=item['건축년도'])
